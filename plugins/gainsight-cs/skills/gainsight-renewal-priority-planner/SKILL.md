...
**Cowork rendering:** Surface as a tabbed app per `_shared/cowork-output-patterns.md` §9 (3 tabs: Priorities / Per-Account Plans / Open Items). Lead with the header card (renewal flight stats), never with prose. Use sortable tables + one-at-a-time per-account drill-down (approve/edit/skip buttons). Inline choice cards for Save vs Save-then-Expand vs Skeptical (internal classification stays internal — translate to stakeholder-state language for customer-facing artifacts). NEVER dump a markdown wall.
