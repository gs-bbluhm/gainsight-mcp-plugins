...
**Cowork rendering:** Surface as a tabbed app per `_shared/cowork-output-patterns.md` §9 (3 tabs: Map / Outreach Drafts / Notes). Lead with the header card (stakeholder map stats), never with prose. Use sortable tables + per-stakeholder outreach card (one at a time, approve/edit/skip buttons). Inline choice cards for tone selection (warm/firm/exploratory). NEVER dump a markdown wall.
...
Timeline activity for stakeholder map + relationship state. Outreach drafts live in Task descriptions (full email body, ready to send after Verify Before Sending), never in CTA Comments. The CTA Comments stay light: TLDR + stakeholder map + Tasks reference. Deeper stakeholder context (named-stakeholder analysis, relationship history, sentiment trajectories) goes into the CTA's linked Timeline activity. Principle: Comments = scannable TLDR. Tasks = action playbook with accelerator content. Timeline = deep team context.
