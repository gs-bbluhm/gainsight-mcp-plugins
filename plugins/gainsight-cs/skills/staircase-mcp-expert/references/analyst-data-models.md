...
| Expansion Analysis | **Rich** (header + per-opp drill-down) | Yes (per opportunity) | Expansion play prep — `gainsight-exec-renewal-radar`, `gainsight-renewal-priority-planner`, `_experimental/staircase-expansion-scout` |
| Risk Analysis | **Rich** (reasons + playbook + stakeholders) | Yes (per risk reason) | Save plays — `gainsight-renewal-priority-planner`, `_experimental/staircase-at-risk-renewals` |
| Churn Analysis | Simple (summary + issues) | Yes (refs at bottom) | Churn retrospective — `gainsight-exec-churn-retrospective` |
